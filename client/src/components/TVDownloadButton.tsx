...
import { Download, Loader2, XCircle, X, ChevronDown, LogIn } from "lucide-react";
...
export default function TVDownloadButton({ showTitle, currentSeason = 1, currentEpisode = 1 }: Props) {
  ...
  }, [status, showTitle, currentSeason, isAuthenticated]);

  const handleDownload = useCallback(async (ep: Episode) => {
    setDownloadProgress(prev => ({ ...prev, [ep.resourceId]: 0 }));
    setShowModal(false);
    try {
      const response = await fetch(ep.download_url);
      if (!response.ok) throw new Error("Download failed");
      const contentLength = response.headers.get("content-length");
      const total = contentLength ? parseInt(contentLength) : ep.size;
      ...
      while (true) {
        ...
        setDownloadProgress(prev => ({ ...prev, [ep.resourceId]: Math.round((received / total) * 100) }));
      }
      const blob = new Blob(chunks);
      const url = URL.createObjectURL(blob);
      const a = document.createElement("a");
      a.href = url;
      a.download = `${showTitle} S${ep.season}E${ep.episode} - ${ep.title}.mp4`;
      a.click();
      URL.revokeObjectURL(url);
    } catch {
      const a = document.createElement("a");
      a.href = ep.download_url;
      a.download = `${showTitle} S${ep.season}E${ep.episode}.mp4`;
      a.click();
    } finally {
      setTimeout(() => setDownloadProgress(prev => { const next = { ...prev }; delete next[ep.resourceId]; return next; }), 2000);
    }
  }, [showTitle]);
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm px-4">
          <div className="bg-[#0a0f1a] border border-white/10 rounded-2xl w-full max-w-sm shadow-2xl overflow-hidden flex flex-col max-h-[80vh]">
            <div className="flex items-center justify-between px-5 py-4 border-b border-white/10 shrink-0">
              <div>
                <p className="text-white font-bold text-sm">{!isAuthenticated ? "Sign in Required" : "Download Episode"}</p>
            ...
            {!isAuthenticated ? (
              <div className="p-6 flex flex-col items-center gap-4">
                <p className="text-[#8899AA] text-sm text-center">You need to sign in to download episodes.</p>
                <button onClick={() => { signIn(); setShowModal(false); }}
                  className="flex items-center gap-2 px-6 py-3 rounded-xl bg-[#00D4FF] text-[#050816] font-bold text-sm hover:scale-105 transition-all">
              ...
              <>
                ...
                <div className="overflow-y-auto flex-1 p-3 flex flex-col gap-2">
                  {filteredEpisodes.map(ep => (
                    <button key={ep.resourceId} onClick={() => handleDownload(ep)}
