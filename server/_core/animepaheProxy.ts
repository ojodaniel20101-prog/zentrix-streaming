...
// ── Route all OmniSave API calls through septorch proxy ───────────────────
async function apiGet(path: string): Promise<any> {
  const targetUrl = `${API_BASE}${path}`;
  const proxyUrl = `${SEPTORCH_PROXY}?url=${encodeURIComponent(targetUrl)}&apikey=${SEPTORCH_API_KEY}`;
...
async function apiPost(path: string, body: any): Promise<any> {
  const targetUrl = `${API_BASE}${path}`;
  const proxyUrl = `${SEPTORCH_PROXY}?url=${encodeURIComponent(targetUrl)}&apikey=${SEPTORCH_API_KEY}&method=POST`;
  
  // Try POST via proxy first, fallback to direct
  try {
...
async function searchAnime(keyword: string) {
  let data: any;
  try {
    data = await apiPost("/wefeed-h5api-bff/subject/search", { keyword, page: 1, perPage: 20 });
  } catch {
