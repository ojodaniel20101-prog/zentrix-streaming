...
export function registerAnimePaheProxy(app: express.Express) {
   ...
   * Query params: anime_title, episode_number, quality (optional)
   */
  app.get("/api/download/anime", async (req, res) => {
    try {
      ...
      console.log(`[OmniSave] Resolved → Season ${location.season}, Episode ${location.episode}`);
   ...
   * Proxies the CDN file through Railway with axios streaming + forced download headers
   * Query params: url (CDN URL), fn (filename)
   */
  app.get("/api/download/anime/file", async (req, res) => {
    try {
      ...
      if (!cdnUrl.startsWith("https://") || !cdnUrl.includes("hakunaymatata.com")) {
        return res.status(400).json({ error: "Invalid CDN URL" });
      }

      const safeFilename = (filename as string) || "episode.mp4";
      console.log(`[OmniSave] Proxying: ${safeFilename}`);
      ...
      return res.status(status).json({ error: error.message || "Proxy failed" });
    }
  });
}
