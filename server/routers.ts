...
// Start sports background sync
sports.startSportsAutoUpdate();
...
export const appRouter = router({
  ...
  sports: router({
    list: publicProcedure
      ...
      .query(async ({ input }) => {
        try { return await sports.listEvents(input); }
        catch (e) { console.error("[sports.list]", e); return []; }
      }),

    live: publicProcedure.query(async () => {
      try { return await sports.getLiveEvents(); }
      catch (e) { console.error("[sports.live]", e); return []; }
    }),

    upcoming: publicProcedure
      .input(z.object({ limit: z.number().min(1).max(100).default(30) }))
      .query(async ({ input }) => {
        try { return await sports.getUpcomingEvents(input.limit); }
        catch (e) { console.error("[sports.upcoming]", e); return []; }
      }),

    replays: publicProcedure
      .input(z.object({ limit: z.number().min(1).max(100).default(30) }))
      .query(async ({ input }) => {
        try { return await sports.getFinishedEvents(input.limit); }
        catch (e) { console.error("[sports.replays]", e); return []; }
      }),

    byId: publicProcedure
      .input(z.object({ id: z.number() }))
      .query(async ({ input }) => {
        try { return await sports.getEventById(input.id); }
        catch (e) { console.error("[sports.byId]", e); return null; }
      }),
    ...
    upsert: adminProcedure
      .input(z.object({
        ...
        status: z.enum(["live", "upcoming", "finished"]).default("upcoming"),
        embedUrl: z.string().optional(),
        youtubeVideoId: z.string().optional(),
      }))
      .mutation(async ({ input }) => {
        try {
          const id = await sports.upsertEvent({
            ...input,
            startTime: input.startTime ? new Date(input.startTime) : new Date(),
          } as any);
          return { success: true, id };
        } catch (e) {
          console.error("[sports.upsert]", e);
    ...
    delete: adminProcedure
      .input(z.object({ id: z.number() }))
      .mutation(async ({ input }) => {
        try { await sports.deleteEvent(input.id); return { success: true }; }
        catch (e) { console.error("[sports.delete]", e); return { success: false }; }
      }),
    ...
    addStream: adminProcedure
      ...
      .mutation(async ({ input }) => {
        try { await sports.addStream(input); return { success: true }; }
        catch (e) { console.error("[sports.addStream]", e); return { success: false }; }
      }),

    removeStream: adminProcedure
      .input(z.object({ streamId: z.number() }))
      .mutation(async ({ input }) => {
        try { await sports.removeStream(input.streamId); return { success: true }; }
        catch (e) { console.error("[sports.removeStream]", e); return { success: false }; }
      }),
  }),
});
