...
export default function MovieDownloadButton({ movieTitle }: Props) {
  ...
  }, [status, movieTitle, isAuthenticated]);
  ...
  const label = () => {
    if (!isAuthenticated) return "Download";
    switch (status) {
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm px-4">
          <div className="bg-[#0a0f1a] border border-white/10 rounded-2xl w-full max-w-sm shadow-2xl overflow-hidden">
            <div className="flex items-center justify-between px-5 py-4 border-b border-white/10">
              <div>
                <p className="text-white font-bold text-sm">{!isAuthenticated ? "Sign in Required" : "Download Movie"}</p>
            ...
            {!isAuthenticated ? (
              <div className="p-6 flex flex-col items-center gap-4">
                <p className="text-[#8899AA] text-sm text-center">You need to sign in to download movies.</p>
