...
export function registerAnimePaheProxy(app: express.Express) {
   ...
   * Proxies the CDN MP4 through our server to bypass ACCESS DENIED.
   * The hakunaymatata CDN blocks direct browser access — requests must
   * include the correct Referer header, which only works server-side.
   *
   * Query params: url (string) — the pre-signed CDN URL
   */
  app.get("/api/download/anime/file", async (req, res) => {
    try {
      ...
      // Only allow hakunaymatata CDN URLs for security
      if (!url.startsWith("https://") || !url.includes("hakunaymatata.com")) {
        return res.status(400).json({ error: "Invalid CDN URL" });
      }

      console.log(`[OmniSave] Proxying file download...`);
      ...
      if (!fileRes.ok) {
        console.error(`[OmniSave] CDN responded: ${fileRes.status}`);
        return res.status(fileRes.status).json({ error: "CDN access denied — URL may have expired" });
      }
      ...
      // Stream directly to the user — no buffering in memory
      if (fileRes.body) {
        ...
        return res.status(500).json({ error: "Empty response from CDN" });
      }
    } catch (error: any) {
      console.error("[OmniSave] File proxy error:", error);
