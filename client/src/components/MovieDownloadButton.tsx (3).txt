...
export default function MovieDownloadButton({ movieTitle }: Props) {
  ...
  const handleClick = useCallback(async () => {
    ...
    try {
      // Step 1: Search
      const searchRes = await fetch(`/api/septorch/search?q=${encodeURIComponent(movieTitle)}`);
      if (!searchRes.ok) throw new Error(`Search failed: HTTP ${searchRes.status}`);
      const searchData = await searchRes.json();
      const items = searchData.data?.items || [];
      if (!items.length) throw new Error(`"${movieTitle}" not found`);

      // Pick best match (movies only: subjectType 1)
      const movies = items.filter((r: any) => r.subjectType === 1);
      const match = movies.find((r: any) =>
        r.title?.toLowerCase().includes(movieTitle.toLowerCase())
      ) || movies[0] || items[0];

      const { subjectId, detailPath } = match;
      if (!subjectId || !detailPath) throw new Error("Could not get movie ID");

      setStatus("resolving");

      // Step 2: Get sources
      const mediaRes = await fetch(
        `/api/septorch/media?subjectId=${subjectId}&detailPath=${encodeURIComponent(detailPath)}&season=0&episode=0`
      );
  ...
  const handleDownload = useCallback(async (src: Source, idx: number) => {
    ...
    try {
      ...
      a.href = url;
      a.download = `${movieTitle} - ${src.resolution}p.${src.format}`;
      a.click();
