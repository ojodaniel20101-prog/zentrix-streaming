...
const firebaseConfig = {
  apiKey: "AIzaSyDPpasgq0M-WvQCUQSZ8jSyCWAQIgiVD78",
  authDomain: "unknown-c1de0.firebaseapp.com",
  projectId: "unknown-c1de0",
  storageBucket: "unknown-c1de0.firebasestorage.app",
  messagingSenderId: "110250184716",
  appId: "1:110250184716:web:51ad426816dad144979e29",
  measurementId: "G-S5ZK6L2Z88"
};
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  // Must await backend auth BEFORE reloading so the session cookie is set
  useEffect(() => {
    getRedirectResult(auth)
      .then(async (result) => {
        ...
        if (ok) {
          ...
          console.error("[Auth] Backend auth failed after redirect");
          setIsLoading(false);
        }
      })
      .catch((err) => {
  ...
  const signInWithManus = useCallback(async () => {
    ...
    try {
      ...
      // Popup succeeded — send token to backend then reload
      const idToken = await result.user.getIdToken();
      const ok = await authenticateWithBackend(idToken);
      if (ok) {
        await new Promise(r => setTimeout(r, 300));
        ...
        console.error("[Auth] Backend auth failed after popup");
        setIsLoading(false);
      }
    } catch (error) {
